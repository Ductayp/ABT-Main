...
function count(t)
	local c = 0
	for i,v in next, t do c = c + 1 end
	return c
end
...
ReplicationRemote.OnClientEvent:Connect(function(Status, Data)
	...
	if Status == 'Connect' then
		...
		print('Caster for connect', Caster, Data.Id)
		ClientCasters[Data.Id] = Caster
		Caster:Start()
	elseif Status == 'Disconnect' then
		local Caster = ClientCasters[Data.Id]
		print('Caster for disconnect', Caster, Data.Id)
		if Caster then
			Caster:Destroy()
			Caster = nil
			ClientCasters[Data.Id] = nil
			print(count(ClientCast.InitiatedCasters), count(ClientCasters))
		end
	end
end)